...
const TablaAnsioliticosPerros = ({ 
  ...
  if (loading) {
    return <Loader text="Cargando ansiolíticos para perros..." />;
  }
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-800">
              Ansiolíticos para Perros ({ansioliticos.length})
            </h3>
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {ansioliticos.map((ansiolitico) => {
                const dosisMinMgKg = dosisMgKg(peso, ansiolitico.dosis.min);
                const dosisMaxMgKg = ansiolitico.dosis.max ? dosisMgKg(peso, ansiolitico.dosis.max) : null;
                
                const fraccionTableta1Min = fraccionTableta(dosisMinMgKg, ansiolitico.tabletas[0]);
                const fraccionTableta1Max = dosisMaxMgKg ? fraccionTableta(dosisMaxMgKg, ansiolitico.tabletas[0]) : null;
                
                const fraccionTableta2Min = fraccionTableta(dosisMinMgKg, ansiolitico.tabletas[1]);
                const fraccionTableta2Max = dosisMaxMgKg ? fraccionTableta(dosisMaxMgKg, ansiolitico.tabletas[1]) : null;
                ...
                return (
                  <tr key={ansiolitico.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 text-center">
                      <div className="font-medium">{ansiolitico.dosis.min} mg/kg</div>
                      {ansiolitico.dosis.max && (
                        <div className="text-xs text-gray-500">
                          hasta {ansiolitico.dosis.max} mg/kg
                        </div>
                      )}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-700 text-center">
                      <div className="font-medium">{ansiolitico.tabletas[0]} mg</div>
                      <div className="text-xs text-gray-500">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 text-center">
                      <div className="space-y-2">
                        <div className="flex items-center justify-center space-x-2">
                          ...
                          <span className="font-medium text-green-600">
                            {formatearDecimal(fraccionTableta1Min)}
                          </span>
                        </div>
                        {fraccionTableta1Max && (
                          <div className="flex items-center justify-center space-x-2">
                            ...
                            <span className="text-xs text-gray-500">
                              {formatearDecimal(fraccionTableta1Max)}
                            </span>
                          </div>
                        )}
                      </div>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700 text-center">
                      <div className="space-y-2">
                        <div className="flex items-center justify-center space-x-2">
                          ...
                          <span className="font-medium text-green-600">
                            {formatearDecimal(fraccionTableta2Min)}
                          </span>
                        </div>
                        {fraccionTableta2Max && (
                          <div className="flex items-center justify-center space-x-2">
                            ...
                            <span className="text-xs text-gray-500">
                              {formatearDecimal(fraccionTableta2Max)}
                            </span>
                          </div>
                        )}
                      </div>
...
export default TablaAnsioliticosPerros; 